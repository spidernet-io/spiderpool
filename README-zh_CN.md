...
* 高性能网络需求的应用，underlay 网络方案能提供低网络延时、高吞吐量的优势。

* 传统的主机应用，如通过主机 IP 来进行服务暴露、基于 VLAN 子网的业务分离等，它们在上云初期，underlay 网络方案能够提供较低的云化网络迁移成本。

* 数据中心网络管理的需求，例如使用防火墙、VLAN 隔离等手段实施网络安全管控，例如使用传统的网络观测手段实施集群网络监控。

* underlay 网络方案能够灵活定制应用接入的 VLAN 子网，不同业务的应用因此可实施独立的网络规划，保障底层子网的业务带宽隔离，适用于如 [kubevirt](https://github.com/kubevirt/kubevirt) 、CSI 存储项目、日志采集项目等。
...
* 支持以最佳实践方式来便捷生成 [Multus](https://github.com/k8snetworkplumbingwg/multus-cni) CR 实例，避免了人工书写 CNI 配置错误。[例子](./docs/concepts/mulltus-zh_CN.md)
