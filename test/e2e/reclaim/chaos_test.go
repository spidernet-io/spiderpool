	...
	Context("Chaos Testing of GC", func() {
		...
		It("Multiple resource types compete for a single IPPool. In scenarios of creation, scaling up/down, and deletion, GC all can correctly handle IP addresses.", Serial, Label("G00012"), func() {
			...
			for name, scaleFunc := range scaleFuncs {
				currentIndex := i
				currentScaleFunc := scaleFunc // Create a new variable to capture the current scaleFunc
				i++
				go func(name string, currentIndex int, scaleFunc error) {
					defer GinkgoRecover()
					defer scaleWg.Done()
					err := scalePodSet(currentScaleFunc, name, int32(randNumbers[currentIndex]), 30*time.Second)
					Expect(err).NotTo(HaveOccurred())
					GinkgoWriter.Printf("Succeed to scale %s/%s replicas to %d \n", chaosNamespace, name, randNumbers[currentIndex])
				}(name, currentIndex, currentScaleFunc(int32(randNumbers[currentIndex])))
			}
			scaleWg.Wait()
