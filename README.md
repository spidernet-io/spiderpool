...
* support to fix the MAC address of pod NIC according to assiged IP, which makes the switch avoid the issue of updating ARP table not in time when pod is restarting. See [doc](https://github.com/spidernet-io/spiderpool/blob/main/docs/usage/coordinator-zh_CN.md#支持固定-pod-的-mac-地址前缀) for details.
