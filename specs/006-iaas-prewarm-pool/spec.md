...
An external IaaS provider component prewarms individual IPs within an IaaS pool asynchronously and records per-IP metadata (paired v6 address, MAC, VLAN, pool-level parent NIC) in the pool's `status.ipMetaData`; presence of a metadata entry IS the readiness state. When Spiderpool's IPAM allocates an IP from such a pool, it must only pick IPs that have a metadata entry and are unclaimed, and — for paired pools — must allocate the whole matched IP pair atomically as one indivisible unit rather than selecting the v4 and v6 addresses independently.
...
- **FR-008**: System MUST extend `SpiderIPPool` status with a single cloud-neutral metadata structure (`status.ipMetaData`) recording: a pool-level parent NIC name; a per-IP `metadata` map keyed by the pool's primary-family address (value carrying the paired IPv6 address, MAC, and VLAN); and two observational counters (`readyIPCount` = IPs with a metadata entry, `unreadyIPCount` = spec.ips IPs without one). Presence of a metadata entry IS the readiness state; there is no separate phase field, failed-IP list, or conditions field.
- **FR-009**: System MUST, during IP candidate selection for a pool carrying the IaaS-provider annotation, only consider an address available if it is present as a key in `status.ipMetaData.metadata` AND does not already appear in the pool's existing `status.allocatedIPs` occupancy record; addresses without a metadata entry, or already recorded as allocated, MUST be treated as unavailable. No separate/new "claimed" marker is added to the metadata entry — occupancy is derived solely from existing allocation bookkeeping. When multiple addresses are simultaneously available, System MUST try them in the same order as Spiderpool's existing (non-IaaS) pool IP candidate ordering convention, without introducing a new selection strategy.
...
- **Per-IP Metadata Entry**: A key/value record within `status.ipMetaData.metadata` representing one prewarmed address (or, for paired pools, one matched v4/v6 address pair with MAC/VLAN). This is the atomic unit of allocation for IaaS pools. It carries no independent occupancy/claim flag — whether an entry is currently claimed is derived by cross-referencing its address(es) against the pool's existing `status.allocatedIPs` record.
