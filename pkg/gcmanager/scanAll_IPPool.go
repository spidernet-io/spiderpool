...
func (s *SpiderGC) executeScanAll(ctx context.Context) {
	...
	// 			if endpoint.Status.Current != nil && endpoint.Status.Current.ContainerID != poolIPAllocation.UID {
	...
	// 					IP:          poolIP,
	// 					ContainerID: poolIPAllocation.UID},
	// 				})
	// 				if nil != err {
...
func (s *SpiderGC) releaseSingleIPAndRemoveWEPFinalizer(ctx context.Context, poolName, poolIP string, poolIPAllocation spiderpoolv1.PoolIPAllocation) error {
	...
	// singleIP := []types.IPAndCID{{IP: poolIP, ContainerID: poolIPAllocation.UID}}
