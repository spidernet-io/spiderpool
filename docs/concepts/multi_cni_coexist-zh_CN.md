...
#### Macvlan Pod 访问 Calico+Macvlan 多网卡 Pod 的 Calico IP

当单 Macvlan Pod 访问 Calico+Macvlan 多网卡 Pod 的 Calico IP 时，结果取决于多网卡 Pod 内部选择的回包路径。

例如，Macvlan Pod 为 `172.100.0.201`，多网卡 Pod 的网络如下：

* Calico 网卡 `eth0`: `10.233.106.188`
* Macvlan 网卡 `net1`: `172.100.0.202/16`
* main 路由表保留 Macvlan 直连子网路由：`172.100.0.0/16 dev net1`

请求报文会通过 Calico 路径进入多网卡 Pod：

    172.100.0.201 -> 10.233.106.188

但是响应报文会匹配 Pod main 路由表中的直连路由，并从 `net1` 发出：

    10.233.106.188 -> 172.100.0.201 dev net1

此时 Macvlan Pod 可以收到 SYN-ACK，并继续发送 ACK 和 HTTP 数据。真正的丢包点发生在多网卡 Pod 所在节点上：ACK/HTTP 报文通过 Calico tunnel 回到该节点后，在进入 Pod 的 Calico workload 链之前被丢弃。该节点上的连接可能停留在 `SYN_SENT [UNREPLIED]`，报文可能命中类似如下规则：

    cali-tw-<workload> -m conntrack --ctstate INVALID -j DROP

这与上文的 kube-proxy `KUBE-FORWARD` INVALID drop 问题不同。设置 `net.netfilter.nf_conntrack_tcp_be_liberal=1` 并重启 kube-proxy 可以移除 kube-proxy 的规则，但不会移除 Calico workload endpoint 的 INVALID 检查。

可以通过以下方式规避该场景：

* 当客户端与多网卡 Pod 的 Macvlan 网卡在同一 Underlay 子网时，优先访问多网卡 Pod 的 Macvlan IP。
* 优先通过 Service 访问多网卡 Pod。ClusterIP 访问经过 kube-proxy 处理，在该拓扑中验证可以正常工作。
* 如果必须支持这种非对称的 Pod IP 直连路径，可以设置 Calico Felix 配置 `disableConntrackInvalidCheck: true`。该配置会移除 Calico workload endpoint 的 `ctstate INVALID` drop 规则，但这是集群级别的 Calico 行为变更，生产环境启用前需要评估影响。
...
下面总结 Calico、Macvlan、Calico+Macvlan 多网卡 Pod 的连通性验证和排障结论。

| 场景 | 结果 | 不通时的丢包点 | 规避方式或前置条件 |
|-|-|-|-|
| Calico Pod 访问 Calico Pod IP | ✅ 通 | N/A | 无需额外配置。 |
| Macvlan Pod 访问同子网 Macvlan Pod IP | ✅ 通 | N/A | 使用 Macvlan 直接转发。 |
| 本地节点访问 Macvlan Pod IP，或 Macvlan Pod 访问本地节点 | ✅ 通 | N/A | 依赖 Spiderpool 在 Pod 和本地节点之间调谐 veth 路由。 |
| Macvlan Pod 访问 Calico Pod IP | ✅ 受影响集群应用 kube-proxy 规避配置后可通 | kube-proxy 可能安装 `KUBE-FORWARD` `ctstate INVALID -j DROP`，并在 TCP 路径不一致时丢包。 | 在每个节点设置 `net.netfilter.nf_conntrack_tcp_be_liberal=1` 并重启 kube-proxy。Kubernetes v1.29 或更高版本在该 sysctl 开启后，kube-proxy 才会跳过安装 INVALID drop 规则。 |
| Macvlan Pod 访问 Calico Pod 的 Service | ✅ 受影响集群应用 kube-proxy 规避配置后可通 | 同样可能受 kube-proxy INVALID drop 问题影响。 | 使用相同的 kube-proxy 规避配置。 |
| Calico Pod 访问 Macvlan Pod IP | ⚠️ 与拓扑相关 | 在 kind 中，跨节点 Pod IP 直连可能因为请求和回复路径不对称而超时。 | Calico 到 underlay 的访问优先使用 Service，或针对实际节点和 Underlay 路由拓扑单独验证。 |
| Calico Pod 访问 Macvlan Pod 的 Service | ⚠️ 单 underlay 后端与拓扑相关 | 在 kind 中，跨节点访问单 underlay 后端的 Service 流量可能被 SNAT 成每节点复用的 veth peer 地址，并在后端节点上被 reset。 | 在归因到路由调谐前，需要验证具体 Service 路径。 |
| Calico+Macvlan 多网卡 Pod 访问 Calico Pod IP | ✅ 通 | N/A | 使用 Calico 网卡和 Calico 路由。 |
| Calico+Macvlan 多网卡 Pod 访问 Macvlan Pod IP | ✅ 通 | N/A | 使用 Macvlan 网卡和 Macvlan 直连子网路由。 |
| Macvlan Pod 访问 Calico+Macvlan 多网卡 Pod 的 Macvlan IP | ✅ 通 | N/A | 当客户端与多网卡 Pod 在同一 Macvlan 子网时，优先使用该路径。 |
| Macvlan Pod 访问 Calico+Macvlan 多网卡 Pod 的 Service | ✅ 通 | N/A | 当客户端不应该依赖某个 Pod 网卡 IP 时，优先使用 Service。 |
| Macvlan Pod 访问同 Macvlan 子网内 Calico+Macvlan 多网卡 Pod 的 Calico IP | ⚠️ 可能不通 | ACK/HTTP 报文通过 Calico tunnel 回到多网卡 Pod 所在节点后，可能被 Calico workload endpoint 规则按 `ctstate INVALID` 丢弃，无法送达 Pod。 | 优先访问多网卡 Pod 的 Macvlan IP 或 Service。如果必须直连 Calico IP，可在评估集群级影响后设置 Calico Felix `disableConntrackInvalidCheck: true`。 |
| 节点或同 Underlay 网络的外部客户端访问 Calico+Macvlan Pod 的 NodePort | ✅ 通，前提是路由已调谐 | 如果没有策略路由调谐，回包可能从错误的 Pod 网卡发出。 | 依赖 Spiderpool 为 NodePort 回包流量调谐策略路由。 |
| 节点或同 Underlay 网络的外部客户端访问单 Macvlan Pod 的 NodePort | ⚠️ 与拓扑相关 | 结果取决于接收 NodePort 流量的节点以及后端 Pod 所在节点。 | 在归因到路由调谐前，需要验证具体节点和后端位置。 |

下面是 underlay 策略路由表不再复制同步的 Kubernetes 或 hijack 路由后的验证结果：

| 测试项 | 结果 | 说明 |
|-|-|-|
| 单 underlay Pod 路由状态 | table 100 只保留 veth default 路由。单 underlay 模式不会添加 `from <underlay-ip> lookup 100` 规则。 | 这符合单 underlay 行为：是否向 table 100 复制路由与是否创建 underlay 源地址规则是两个独立问题。 |
| 单 underlay Pod 访问 Calico Pod IP 或 Calico Service | ✅ 通 | 已在应用 kube-proxy INVALID 规避配置后的同节点源 Pod 上验证。 |
| Calico 或 Calico+Macvlan Pod 访问单 underlay Pod IP | ⚠️ 与拓扑相关 | 同节点访问可通；在 kind 拓扑中跨节点 Pod IP 直连超时。建议优先使用 Service。 |
| Calico 或 Calico+Macvlan Pod 访问单 underlay Pod Service | ⚠️ 与拓扑相关 | 抓包显示请求被 SNAT 成每节点复用的 veth peer 地址，后端节点 reset 了 SYN-ACK 路径。 |
| 节点或外部 underlay 客户端访问单 underlay Pod NodePort | ⚠️ 与拓扑相关 | 结果取决于接收 NodePort 的节点和后端位置。没有完整验证报文路径前，不应把已观察到的现象归因于路由调谐。 |
| Calico+Macvlan Pod 路由状态 | table 100 只保留 net1 default 和直连子网路由。main 表仍保留 underlay 直连子网路由。 | 带网关的自定义路由不会被“直连路由保留”逻辑留在 main 表。 |
| Macvlan Pod 访问 Calico+Macvlan Pod 的 Macvlan IP 或 Service | ✅ 通 | 已验证 Calico+Macvlan Pod 的 underlay 网卡在 `172.100.0.0/16`。 |
| Macvlan Pod 访问同 Macvlan 子网内 Calico+Macvlan Pod 的 Calico IP | ⚠️ 可能不通 | 该失败由 Calico workload endpoint 的 `ctstate INVALID` 处理导致。 |

说明：

* ✅: 经验证或配合文档中的 Spiderpool 路由调谐后预期可以正常通信。
* ⚠️: 需要满足额外条件或增加配置后才能稳定通信。
* kube-proxy INVALID drop 规避配置和 Calico `disableConntrackInvalidCheck: true` 解决的是不同丢包点。前者移除 kube-proxy 的 `KUBE-FORWARD` INVALID drop 规则；后者移除 Calico workload endpoint 的 INVALID drop 规则。
* underlay 策略路由表不会接收同步的 Kubernetes 或 hijack 路由。在单 underlay 模式下，已测试的 Pod IP 和 Service 路径没有观察到由该路由表行为单独导致的连通性差异；在归因 Service 或 NodePort 失败前，需要先完整验证报文路径。
