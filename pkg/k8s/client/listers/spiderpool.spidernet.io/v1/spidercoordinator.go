...
type SpiderCoordinatorLister interface {
	...
	List(selector labels.Selector) (ret []*v1.SpiderCoordinator, err error)
	// Get retrieves the SpiderCoordinator from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.SpiderCoordinator, error)
...
// List lists all SpiderCoordinators in the indexer.
func (s *spiderCoordinatorLister) List(selector labels.Selector) (ret []*v1.SpiderCoordinator, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SpiderCoordinator))
	})
	return ret, err
}

// Get retrieves the SpiderCoordinator from the index for a given name.
func (s *spiderCoordinatorLister) Get(name string) (*v1.SpiderCoordinator, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("spidercoordinator"), name)
	}
	return obj.(*v1.SpiderCoordinator), nil
}
