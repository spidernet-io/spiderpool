...
| `plugins.image.tag`              | the image tag of plugins                                                       | `adb64e204bbae0fb443185b4eb54c786bb492d55`   |
...
| `spiderpoolController.podResourceInject.namespacesExclude`                      | exclude the namespaces of the pod resource inject                                                                                                                                               | `["kube-system","spiderpool","spiderpool-system","metallb-system","istio-system"]` |
...
| Name                                         | Description                                                                                                                                                                                                                       | Value                          |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `iaasNetworkProvider.service.name`           | the Service name of the IaaS provider. If empty, IaaS integration is disabled.                                                                                                                                                    | `""`                           |
| `iaasNetworkProvider.service.namespace`      | the namespace of the IaaS provider Service (its TLS Secret is expected in the same namespace).                                                                                                                                    | `iaas-network-provider-system` |
| `iaasNetworkProvider.service.port`           | the port of the IaaS provider Service.                                                                                                                                                                                            | `8443`                         |
| `iaasNetworkProvider.tls.caSecret`           | the name of the provider TLS Secret (in service.namespace). At install/upgrade time helm looks it up and copies only its ca.crt into a local Secret 'iaas-provider-ca' (never tls.key).                                           | `iaas-network-provider-tls`    |
| `iaasNetworkProvider.tls.ca`                 | base64-encoded PEM CA bundle used to verify the provider serving certificate. Takes precedence over the caSecret lookup. Required for GitOps or 'helm template' where lookup is unavailable.                                      | `""`                           |
| `iaasNetworkProvider.tls.insecureSkipVerify` | skip provider certificate verification. Gradual-rollout fallback only, not recommended.                                                                                                                                           | `false`                        |
...
| `iaasNetworkProvider.excludeReportNics`      | local physical NIC names (e.g. management or storage NICs) that spiderpool-agent must not report to the Node annotation ipam.spidernet.io/parent-nics                                                                             | `[]`                           |
