...
func (adc *allocationDurationConstruct) RecordIPAMAllocationDuration(ctx context.Context, allocationDuration float64) {
	...
	go func() {
		...
		ipamAllocationLatestDurationSeconds.Add(ctx, allocationDuration)
		...
		ipamAllocationAverageDurationSeconds.Add(ctx, adc.allocationAvgDuration)
		...
		if allocationDuration > adc.maxAllocationDuration {
			...
			ipamAllocationMaxDurationSeconds.Add(ctx, adc.maxAllocationDuration)
		}
		...
		if adc.allocationCounts == 1 || allocationDuration < adc.minAllocationDuration {
			...
			ipamAllocationMinDurationSeconds.Add(ctx, adc.minAllocationDuration)
		}
...
func (ddc *releaseDurationConstruct) RecordIPAMReleaseDuration(ctx context.Context, releaseDuration float64) {
	...
	go func() {
		...
		ipamReleaseLatestDurationSeconds.Add(ctx, releaseDuration)
		...
		ddc.releaseCounts++
		ipamReleaseAverageDurationSeconds.Add(ctx, ddc.releaseAvgDuration)

		// IPAM maximum release duration
		if releaseDuration > ddc.maxReleaseDuration {
			ddc.maxReleaseDuration = releaseDuration
			ipamReleaseMaxDurationSeconds.Add(ctx, ddc.maxReleaseDuration)
		}
		...
		if ddc.releaseCounts == 1 || releaseDuration < ddc.minReleaseDuration {
			...
			ipamReleaseMinDurationSeconds.Add(ctx, ddc.minReleaseDuration)
		}
