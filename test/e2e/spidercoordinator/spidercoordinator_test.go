	...
	Context("It can get the clusterCIDR from kubeadmConfig and kube-controller-manager pod", func() {
		...
		It("status should be NotReady if neither kubeadm-config configMap nor kube-controller-manager pod can be found", func() {
			...
			Eventually(func() bool {
				...
				GinkgoWriter.Printf("got spidercoordinator's status: %v\n", sp.Status)
			...
			Eventually(func() bool {
				...
				if kcmPod.Status.Phase != corev1.PodRunning {
					return false
				}

				return true
			}, common.EventOccurTimeout, common.ForcedWaitingTime).Should(BeTrue())
		})
	})
