...
5. **Global pool mode** (design: `global-pool-design.md`, spec US4 /
   ...
   (`{scope, ips: {addr: {ipv6, mac, vlan[, node, status, detachTime]}}}`, where
...
(`{scope, ips}`) is the only accepted shape: `scope` is
mandatory and scope-less payloads fail closed (nothing is merged, so no
migration path is needed). The legacy reserved `parentNic` metadata key is
tolerated and ignored; the parent NIC lives in the agent-written
`status.parentNic {name, mac}` field (see spec.md Session 2026-09-21).
...
├── spiderippool_types.go       # + IPMetaData status field (metadata JSON string, observedGeneration, ready/unready counters) + agent-written ParentNic {name, mac}; no Conditions
...
├── metadata_cache.go            # + immutable parsed metadata snapshots keyed by pool UID + observedGeneration; decodes schema v2 {scope, ips} (legacy parentNic key ignored); scope-less payloads fail closed (FR-018)
