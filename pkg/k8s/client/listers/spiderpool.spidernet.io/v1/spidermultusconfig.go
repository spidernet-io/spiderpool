...
type SpiderMultusConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1.SpiderMultusConfig, err error)
...
// List lists all SpiderMultusConfigs in the indexer.
func (s *spiderMultusConfigLister) List(selector labels.Selector) (ret []*v1.SpiderMultusConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SpiderMultusConfig))
	})
	return ret, err
}
...
type SpiderMultusConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.SpiderMultusConfig, error)
...
// List lists all SpiderMultusConfigs in the indexer for a given namespace.
func (s spiderMultusConfigNamespaceLister) List(selector labels.Selector) (ret []*v1.SpiderMultusConfig, err error) {
...
// Get retrieves the SpiderMultusConfig from the indexer for a given namespace and name.
func (s spiderMultusConfigNamespaceLister) Get(name string) (*v1.SpiderMultusConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("spidermultusconfig"), name)
	}
	return obj.(*v1.SpiderMultusConfig), nil
}
