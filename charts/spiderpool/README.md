...
| `iaasNetworkProvider.eniDevPlugin.enabled`               | enable the spiderpool-agent device plugin for auxiliary ENI slot scheduling.                                                                                                                                                      | `false`                |
| `iaasNetworkProvider.eniDevPlugin.resourceName`          | the extended resource name advertised to kubelet.                                                                                                                                                                                 | `spidernet.io/sub-eni` |
| `iaasNetworkProvider.eniDevPlugin.maxSlotsPerNode`       | the total auxiliary ENI slot capacity advertised on each node.                                                                                                                                                                    | `0`                    |
| `iaasNetworkProvider.eniDevPlugin.kubeletRootDir`        | kubelet root directory used to derive device-plugin and plugin-registration host paths.                                                                                                                                           | `/var/lib/kubelet`     |
| `iaasNetworkProvider.eniDevPlugin.injectPodENIResources` | enable webhook injection of ENI slot resource requests for eligible provider-mode VLAN Pods.                                                                                                                                      | `true`                 |
