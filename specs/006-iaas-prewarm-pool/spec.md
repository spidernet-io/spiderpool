...
### Session 2026-09-21 (parent NIC via structured status — status.parentNic)

- Q: Does the agent still need to report the whole physical-NIC inventory of every node to the Node annotation `ipam.spidernet.io/parent-nics`? → A: No. The parent NIC moves into the pool object: the `ipam.spidernet.io/parent-nic` annotation (required on node-level IaaS pools, optional on global pools) names the NIC; the spiderpool-agent on the pool's node (node-level pools only) resolves its MAC locally via netlink and publishes both to the new agent-owned `status.parentNic {name, mac}` field, which the provider reads instead of the Node annotation. The Node annotation, its `excludeReportNics` config, and the agent's nodes patch/update RBAC are removed. The reserved `parentNic` key inside `ipMetaData.metadata` is no longer written by providers and is tolerated/ignored by readers. Runtime MAC resolution order becomes: subnet cache → `status.parentNic.mac` (node-level pools) → `parent-nic` annotation + local netlink → SpiderMultusConfig master fallback (global pools without the annotation; MACs differ per node). To keep `status.parentNic` a single value, an IaaS node-level pool is restricted to exactly one `spec.nodeName` entry (webhook-enforced); for paired dual-stack sets only the primary (v4) pool carries `status.parentNic`, mirroring `ipMetaData`.

### Session 2026-09-04 (IaaS-provider annotation semantics — pool mode derived from `spec.nodeName`)

- Q: Does the `ipam.spidernet.io/iaas-provider` annotation mean "prewarm this pool"? → A: No. The annotation carries exactly two meanings: (1) the pool is taken under management by the named IaaS provider, and (2) Spiderpool IPAM applies the IaaS allocation behavior (metadata gating, pair atomicity, skip of the legacy synchronous call) to it. Whether and when prewarming happens is decided by the pool's own shape, not by any annotation: `spec.nodeName` set → node-level pool, the provider prewarms it; `spec.nodeName` empty → global pool, no prewarming (first-use creation + sticky cache). Consequently the dedicated `ipam.spidernet.io/iaas-global: "true"` annotation, its mirrored label, and its webhook validation are removed, and FR-019 is reversed (recognition IS inferred from the IaaS-provider annotation plus an empty `spec.nodeName`). To keep a pool's mode stable for its lifetime, the validating webhook rejects adding or removing `spec.nodeName` on an IaaS pool after creation.
...
- Q: Does the metadata schema need per-IP placement? → A: Yes, schema v2: `{"scope": "<nodeName>"|"", "parentNic": "<nic>", "ips": {addr: {ipv6, mac, vlan[, node, status, detachTime]}}}`. `scope` is mandatory; node-level pools put placement in `scope` (must equal `spec.nodeName`, no per-entry `node`); global pools use an explicit empty `scope` and per-entry `node` (absent = created-but-detached); `status` (when present) is the first-checked lifecycle gate with `node`/`vlan` consistency validation, `detachTime` is observational. Writers emit v2 only; readers reject scope-less payloads (fail closed).
...
- Q: How does the reclaim race guard prevent a Pod from being configured with a detached cloud IP (watermark reclaim may pick an idle IP at the same moment Spiderpool selects it)? → A: The cloud API keeps ip/mac stable across detach but reassigns the VLAN on every attach, so the cached `vlan` is only valid while attached. The provider signals "detaching" by setting the entry's `vlan` to `-1` (one CR write, serialized by resourceVersion against `allocatedIPs` claims) before calling the cloud. Spiderpool MUST treat an entry with `node` present and `vlan == -1` as detaching and skip it in both the hit predicate and cold-path candidates; the hit predicate additionally requires `vlan != -1`. An unbound entry (no `node`) with `vlan == -1` stays a cold-path candidate since the RPC response supplies the authoritative VLAN. Entries may also carry a provider-written `status` enum (`bound`/`unbound`/`detaching`) and a `detachTime` timestamp (reclaim-candidate mark time, `deletionTimestamp`-style): when `status` is present it is checked first (`detaching` never allocatable, `bound` hit/steal, `unbound` cold-path) and `node`/`vlan` must then be consistent with it — a contradiction skips that entry only (per-entry fail closed, error log + metric); when absent, the `vlan == -1` sentinel derivation applies alone.
...
- Q: How is a pool marked as IaaS-managed? → A: Via `ipam.spidernet.io/iaas-provider: "<vendor>"` (annotation, mirrored to a label by the mutating webhook), replacing the boolean `ipam.spidernet.io/iaas-pool: "true"`. The vendor value is opaque to Spiderpool — only the annotation's presence matters; the value is never validated.
...
`vlan != -1`) allocates with zero provider RPC using cached
...
5. **Given** a metadata entry whose `node` is present and `vlan` is `-1`
   (the provider's reclaim race guard set the detaching sentinel before
   detaching — detach preserves ip/mac but invalidates the VLAN), **When**
   the agent computes hit or cold-path candidate sets, **Then** that entry
   is skipped in both, so a Pod can never be configured with the stale VLAN
   of a detached sub-ENI; an unbound entry (no `node`) with `vlan == -1`
   remains a valid cold-path candidate because the RPC response supplies
   the authoritative VLAN.
...
8. **Given** a pool whose metadata lacks the mandatory `scope` key (including
   the pre-v2 flat shape with top-level address keys), **When** the agent
   decodes it, **Then** decoding fails closed as not-yet-reconciled;
   Spiderpool never writes metadata.
...
- **FR-018**: System MUST decode `status.ipMetaData.metadata` as schema v2 — `{"scope": "<nodeName>"|"", "ips": {addr: {ipv6, mac, vlan[, node, status, detachTime]}}}` (the legacy reserved top-level `parentNic` key is tolerated and ignored; the parent NIC lives in the structured `status.parentNic` field, see Session 2026-09-21). `scope` is mandatory in v2: a node name means a node-level pool (per-entry `node` MUST NOT appear; the value MUST equal `spec.nodeName`), an explicit empty string means a global pool (per-IP placement comes from `ips[addr].node`; a missing `node` means created-but-detached). `vlan == -1` is the detaching/VLAN-unknown sentinel (entries may also carry a provider-written `status` enum checked first when present — with mandatory `node`/`vlan` consistency validation, contradictions skipping that entry only — and an observational `detachTime`). Readers MUST reject any payload without the `scope` key (fail closed, not-yet-reconciled); Spiderpool never writes metadata.
- **FR-019**: System MUST recognize a pool as a global pool when it carries the `ipam.spidernet.io/iaas-provider` annotation AND `spec.nodeName` is empty; an IaaS pool with `spec.nodeName` set is a node-level (prewarm) pool. The IaaS-provider annotation by itself only marks provider management and IaaS IPAM behavior — it never implies prewarming; the prewarm-vs-realtime mode is derived solely from the presence of `spec.nodeName`. No dedicated global-pool marker exists (the former `ipam.spidernet.io/iaas-global` annotation and its mirrored label are removed). To keep a pool's mode stable for its lifetime, the validating webhook MUST reject adding or removing `spec.nodeName` on an IaaS pool after creation. Node-level pools MUST keep the exact US1–US3 prewarm behavior; global-pool logic MUST NOT engage for them.
- **FR-020**: For a global pool, the agent's cache-hit predicate MUST be: `effectiveNode(ip) == localNode` AND the address is absent from `status.allocatedIPs` AND `ips[ip].vlan != -1`, where `effectiveNode(ip) = scope != "" ? scope : ips[ip].node`. A hit MUST allocate with zero provider RPC and zero cloud calls, configuring the Pod interface from the cached `{ipv6, mac, vlan}`; the `vlan != -1` condition guarantees the cached VLAN is still that of an attached sub-ENI.
...
- **FR-023**: Agents MUST treat a metadata entry with `node` present and `vlan == -1` as detaching and skip it in both the cache-hit predicate and the cold-path candidate set, so the provider's reclaim race guard (a single CR write setting `vlan: -1`, serialized by object resourceVersion against `allocatedIPs` claims) is honored and a Pod never receives an IP whose cloud sub-ENI is being (or has been) detached with a stale VLAN. Unbound entries (no `node`) with `vlan == -1` remain cold-path candidates; their authoritative VLAN comes from the RPC response.
...
- **FR-026**: Within one Pod interface's pool candidate, pools MUST be narrowed to the highest configured class before any per-Pod filtering: (1) paired IaaS primary pools (dual-stack enabled), (2) IaaS pools (node-level or global, which MAY coexist — node-pinned pools sort first, global pools serve as fallback), (3) plain pools. Lower-class pools MUST be ignored with a warning log and a Pod warning event, and allocation MUST fail rather than fall back to an ignored pool, so IaaS allocation never silently degrades across classes on any node.
...
- **Global Pool**: An IaaS pool (IaaS-provider annotation present) without `spec.nodeName`, whose metadata `scope` is an explicit empty string; recognized solely by this shape — no dedicated annotation. Its sub-ENIs are created on first use, stay bound to a node as a sticky cache after Pod deletion (per-entry `node`), and are reclaimed by the external provider via watermarks. Contrasts with the node-level (prewarm) pool whose placement is the pool-level `scope`/`spec.nodeName`.
