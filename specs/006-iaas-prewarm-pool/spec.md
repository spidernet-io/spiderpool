...
- Q: Does the metadata schema need per-IP placement? → A: Yes, schema v2: `{"scope": "<nodeName>"|"", "parentNic": "<nic>", "ips": {addr: {ipv6, mac, vlan[, node]}}}`. `scope` is mandatory; node-level pools put placement in `scope` (must equal `spec.nodeName`, no per-entry `node`); global pools use an explicit empty `scope` and per-entry `node` (absent = created-but-detached). Writers emit v2 only; readers reject scope-less payloads (fail closed).
...
- **FR-018**: System MUST decode `status.ipMetaData.metadata` as schema v2 — `{"scope": "<nodeName>"|"", "parentNic": "<nic>", "ips": {addr: {ipv6, mac, vlan[, node]}}}`. `scope` is mandatory in v2: a node name means a node-level pool (per-entry `node` MUST NOT appear; the value MUST equal `spec.nodeName`), an explicit empty string means a global pool (per-IP placement comes from `ips[addr].node`; a missing `node` means created-but-detached). `vlan == -1` is the detaching/VLAN-unknown sentinel (no separate `detaching` field exists). Readers MUST reject any payload without the `scope` key (fail closed, not-yet-reconciled); Spiderpool never writes metadata.
