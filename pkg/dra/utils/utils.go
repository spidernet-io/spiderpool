...
func GetStaticNicsFromSpiderClaimParameter(ctx context.Context, apiReader client.Reader, pod *corev1.Pod) ([]spiderpoolv2beta1.StaticNic, error) {
	for _, rc := range pod.Spec.ResourceClaims {
		if rc.Source.ResourceClaimTemplateName != nil {
			...
			if rct.Spec.Spec.ResourceClassName == constant.DRADriverName && rct.Spec.Spec.ParametersRef.APIGroup == constant.SpiderpoolAPIGroup &&
				...
				if err := apiReader.Get(ctx, apitypes.NamespacedName{Namespace: pod.Namespace, Name: rct.Spec.Spec.ParametersRef.Name}, &scp); err != nil {
					return nil, fmt.Errorf("failed to get spiderClaimParameter for resourceClaimTemplate %s/%s: %v", pod.Namespace, rct.Name, err)
				}

				return scp.Spec.StaticNics, nil
			}
		}
	}
...
func GetRdmaResourceMapFromStaticNics(ctx context.Context, apiReader client.Reader, staticNics []spiderpoolv2beta1.StaticNic) (map[string]bool, error) {
	...
	for _, nic := range staticNics {
		...
		if err := apiReader.Get(ctx, apitypes.NamespacedName{Namespace: nic.Namespace, Name: nic.MultusConfigName}, &smc); err != nil {
			return nil, err
		}
