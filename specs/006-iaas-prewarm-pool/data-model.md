...
|---|---|---|---|
| `ipam.spidernet.io/iaas-provider` | string (vendor name, e.g. `"huaweicloud"`) | No | Marks the pool as IaaS-managed by the named provider, i.e. addresses in `spec.ips` require provider prewarm confirmation (a `status.ipMetaData.metadata` entry) before they may be allocated. Absence = pool behaves exactly as today (FR-006). Presence = allocation always intersects with `status.ipMetaData.metadata` keys, regardless of whether that map is currently empty (see §1.3). The validating webhook rejects unsupported vendor values; the currently supported vendor list is `huaweicloud`. |
...
type IPMetaData struct {
    ...
    // Metadata maps an IP address to its metadata entry. The key is the
    // pool's primary-family address: IPv4 for a v4 pool (including the
    // primary pool of a dual-stack pair); IPv6 only for a pure
    // IPv6-single-stack pool that has no v4 side.
    // +kubebuilder:validation:Optional
...
- A `Metadata` map key MUST parse as a valid IP address of the pool's own
...
- For an IaaS-managed pool, Spiderpool computes the normal candidate set first
  ...
  that candidate set with the keys of `status.ipMetaData.metadata`. Only
...
There is no per-entry `Phase` field and no periodic retry of failed addresses
...
SpiderIPPool (IaaS pool, e.g. node1-app-a-v4)
   ...
   │     ├── metadata{ipv4 -> {ipv6,mac,vlan}}  (prewarmed entries; presence = ready)
   ...
   └── IPAM resolves candidates ──> [node1-app-a-v4] ──(auto-complete)──> [node1-app-a-v4, node1-app-a-v6]
              └── selectByPod (nodeName/podAffinity, unchanged) ──> final pool(s)
                     └── AllocateIP driven from v4 (primary) pool's metadata only:
                            normal candidate(v4) ∩ v4pool.ipMetaData.metadata keys
