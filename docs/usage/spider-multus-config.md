...
#### Configure coordinator routes

`spec.coordinator.policyRoutes` configures static routes for the coordinator plugin generated by SpiderMultusConfig. These routes are written into the generated NetworkAttachmentDefinition and are installed by coordinator into the policy routing table of the interface where coordinator is running. If `spec.coordinator.policyRoutes` is not set, coordinator falls back to the global default from `SpiderCoordinator.spec.policyRoutes`.
...
After the SpiderMultusConfig is reconciled, the generated NetworkAttachmentDefinition contains the coordinator route configuration:
...
For each route, `dst` can be an IP or CIDR and `gw` must be an IP address. `dst` and `gw` must use the same IP family.
