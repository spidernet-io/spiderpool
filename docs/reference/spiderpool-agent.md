...
- If Pods remain Pending, check whether their containers request `spidernet.io/eni-slot` and whether any node reports enough allocatable capacity for that resource.
- If nodes do not show `spidernet.io/eni-slot` in `status.allocatable`, verify `iaasNetworkProvider.serverUrl`, `iaasNetworkProvider.eniDevPlugin.enabled`, `iaasNetworkProvider.eniDevPlugin.maxSlotsPerNode`, and `iaasNetworkProvider.eniDevPlugin.kubeletRootDir`, then check spiderpool-agent logs for the selected plugin path and registration failures.
