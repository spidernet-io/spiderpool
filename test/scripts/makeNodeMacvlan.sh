...
IPV4_GW=$( ip r get 8.8.8.8 | grep $INTERFACE | awk '{print $3}' )
IPV6_GW=$( ip r get 2001::1 | grep $INTERFACE | awk '{print $5}' )
...
if [ -z "$DISABLE_IPV6" ] ; then
  ...
  ip -6 r add default via $IPV6_GW dev $CHILD_INTERFACE
fi
