...
func NewDriver(ctx context.Context, podManager podmanager.PodManager, claimManager ResourceClaimManager) error {
	...
	d.draPlugin, err = kubeletplugin.Start(ctx,
		[]any{d},
		kubeletplugin.NodeName(""),
		kubeletplugin.DriverName(""),
		kubeletplugin.RegistrarSocketPath(""),
		kubeletplugin.PluginSocketPath(""),
		kubeletplugin.KubeletPluginSocketPath(""),
	)
...
func (d *driver) nodePrepareResource(ctx context.Context, claim *drapb.Claim) (devices []*drapb.Device, err error) {
	...
	multusConfig, err := GetNetworkConfig(resourceClaim.Spec.Devices.Config)
	if err != nil {
		return nil, fmt.Errorf("failed to get network config from resource claim: %v", err)
	}
	...
	if multusConfig.SecondaryNics != nil {
		if len(multusConfig.SecondaryNics.StaticNics) > 0 {
			// staticNic mode
			annotations = multusConfig.ParseToAnnotations()
		} else if multusConfig.SecondaryNics.DynamicNics != nil {
			//TODO: dynamicNic mode
		}
	}
