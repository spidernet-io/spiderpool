...
func firstInvoke(netns ns.NetNS, mode Mode) (bool, error) {
	switch mode {
	case ModeUnderlay:
		return networking.IsInterfaceMiss(netns, defaultUnderlayVethName)
	case ModeOverlay:
		return networking.IsRuleMiss(netns, defaultPodRuleTable)
	}
	return false, fmt.Errorf("unknown tuneMode: %s", mode)
}
...
// one to container.
func (c *coordinator) setupVeth(containerID string) (net.HardwareAddr, net.HardwareAddr, error) {
	var hostInterface, containerInterface net.Interface
	err := c.netns.Do(func(hostNS ns.NetNS) error {
		var err error
		hostInterface, containerInterface, err = ip.SetupVethWithName(c.podVethName, getHostVethName(containerID), 1500, "", hostNS)
		if err != nil {
			return err
		}
	...
	return hostInterface.HardwareAddr, containerInterface.HardwareAddr, nil
}
...
func (c *coordinator) setupRoutes(logger *zap.Logger, ruleTable int) error {
	...
	err = c.netns.Do(func(_ ns.NetNS) error {
		...
		for _, hijack := range c.HijackCIDR {
			...
			if err := networking.AddRoute(logger, ruleTable, netlink.SCOPE_LINK, c.podVethName, ipNet, v4Gw, v6Gw); err != nil {
