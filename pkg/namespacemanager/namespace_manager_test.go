...
var _ = Describe("NamespaceManager", Label("namespace_manager_test"), func() {
	...
	Describe("Test NamespaceManager's method", func() {
		...
		Describe("MatchLabelSelector", func() {
			It("checks non-existent Namespace", func() {
				ctx := context.TODO()
				match, err := nsManager.MatchLabelSelector(ctx, nsName, &metav1.LabelSelector{MatchLabels: labels})
				Expect(err).NotTo(HaveOccurred())
			...
			It("matches invalid label selector", func() {
				...
				match, err := nsManager.MatchLabelSelector(ctx, nsName, invalidSelector)
				Expect(err).To(HaveOccurred())
				Expect(match).To(BeFalse())
			})

			It("failed to list Namespace due to some unknown errors", func() {
				patches := gomonkey.ApplyMethodReturn(fakeClient, "List", constant.ErrUnknown)
				defer patches.Reset()
			...
			It("matches nothing", func() {
				...
				match, err := nsManager.MatchLabelSelector(ctx, nsName, nil)
				Expect(err).NotTo(HaveOccurred())
				Expect(match).To(BeFalse())
			})
