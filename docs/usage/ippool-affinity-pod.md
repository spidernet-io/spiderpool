...
> Spiderpool supports multiple ways to select ippool. A Pod will select a specific ippool to allocate IP according to the corresponding rules with different priorities. Meanwhile, ippool can use selectors to filter its user.

## Priority

Spiderpool supports the following ways to specify ippool for Pod:

1. Pod annotations (**high priority**): Specifies which ippool the current Pod should use to allocate IP. This takes precedence over any other selection rules.
   ...
   - `ipam.spidernet.io/ippools`: For multiple interfaces case. Note that it does not mean that CNI will return the IP allocation results of multiple interfaces in one request (this will break the [CNI Specification](https://www.cni.dev/docs/spec/)). Spiderpool will allocate multiple IPs in one request, but return them in several times. It is mainly used with [Spiderflat CNI](TODO) and [multus CNI](https://github.com/k8snetworkplumbingwg/multus-cni). You can get more details of "multus + Spiderpool" [here](TODO).
...
More details see Spiderpool [annotation](https://spidernet-io.github.io/spiderpool/usage/annotation/) and [configuration](https://spidernet-io.github.io/spiderpool/usage/config/).

## Backup

Each ippool selection rule supports 'backup'. We can specify multiple ippools in the array to achieve this effect. Spiderpool will successively try to allocate IP in the order of elements in the ippool array until the first allocation succeeds or all fail.

The following is an example of Pod annotation `ipam.spidernet.io/ippool`, and the same is true for other ippool selection rules.
...
As you can see from the above, Pod `sample-backup` will attempt to allocate IP from ippools defined in field `ipv4pools`.
...
You will see Pod `sample-backup` successfully allocated IP from ippool `backup-v4-ippool`.
...
Ippool can also use `nodeSelector`, `namesapceSelector`, and `podSelector` to filter its users. It should be regarded as a **filtering mechanism** rather than an ippool selection rule (refer to [Priority](#priority) for ippool selection rules).
...
Obviously, ippool `master-v4-ippool` only works with the control plane nodes of Kubernetes. The Pod which is scheduled to the master nodes can be correctly allocated IP from this ippool.
...
The ippool `worker-v4-ippool` is the opposite.

Then, you can run the following Deployment `example`, which has 5 replicas. It is expected that the Pods controlled by `example` can be evenly scheduled to master nodes and worker nodes. Of course, maybe you should remove some related Taints.
...
Finally, you will find that Pods at different nodes use different ippools.
