...
- **FR-019**: System MUST recognize a pool as a global pool via the explicit `ipam.spidernet.io/iaas-global: "true"` annotation (mirrored to a label of the same key by the mutating webhook; any other value is rejected by the validating webhook). Recognition MUST NOT be inferred from the IaaS-provider annotation or from an empty `spec.nodeName`. Pools without this marker MUST keep the exact US1–US3 prewarm behavior; global-pool logic MUST NOT engage for them.
